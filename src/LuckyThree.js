...
class LuckyThree extends React.Component {
  ...
  displayRandomNumbers = (e) => {
    ...
    interval_id = setInterval(() => {
      currentTarget.childNodes[0].innerText = Math.floor(
  ...
  checkAllCards = (e) => {
    ...
    for (let i = 1; i < this.props.idArr.length; i++) {
      if (
        document.getElementById(this.props.idArr[i]).innerText !==
        document.getElementById(this.props.idArr[i - 1]).innerText
      ) {
        alert('Ohh! You lost!');
        return;
      }
      ...
      <div className="main-container">
        <div className="number-cards-container">
          {/* DONE 2: 
            ...
            */}
          <NumberCard
            id={this.props.idArr[0]}
            color={this.props.colorsArr[0]}
            cardClickHandler={this.cardClickHandler}
          />
          <NumberCard
            id={this.props.idArr[1]}
            color={this.props.colorsArr[1]}
            cardClickHandler={this.cardClickHandler}
          />
          <NumberCard
            id={this.props.idArr[2]}
            color={this.props.colorsArr[2]}
            cardClickHandler={this.cardClickHandler}
          />
        </div>
        ...
        <input
          ...
          type="button"
          value="Try Your Luck"
          onClick={this.tryLuckClickHandler}
        />
      </div>
    );
  }
